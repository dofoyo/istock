...
@Scope("prototype")
@Component("bav2")
public class SimulateBAV2  implements Simulate{
	protected static final Logger logger = LoggerFactory.getLogger(SimulateBAV2.class);
	...
	public Future<String> run(LocalDate beginDate, LocalDate endDate, BigDecimal initCash, Integer top, boolean isAveValue, Integer quantityType, boolean isEvaluation) throws Exception  {
		...
		Map<String, String> operateResult = operation.run(account, operationList,null, beginDate, endDate, "bav2", top, isAveValue,quantityType);
		turtleSimulationRepository.save("bav2", operateResult.get("breakers"), operateResult.get("CSV"), operateResult.get("dailyAmount"), operateResult.get("dailyHolds"), isEvaluation);
