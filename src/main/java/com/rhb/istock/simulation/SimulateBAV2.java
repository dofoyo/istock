...
public class SimulateBAV2 {
	...
	public Future<String> run(LocalDate beginDate, LocalDate endDate, BigDecimal initCash, Integer top, boolean isAveValue, Integer quantityType, boolean isEvaluation) throws Exception  {
		...
		Map<String, String> operateResult = operation.run(account, operationList, beginDate, endDate, "bav2", top, isAveValue,quantityType);
		turtleSimulationRepository.save("bav2", operateResult.get("breakers"), operateResult.get("CSV"), operateResult.get("dailyAmount"), isEvaluation);
