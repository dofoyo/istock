 ...
 */
public class Tdata {
	protected static final Logger logger = LoggerFactory.getLogger(Tdata.class);
	...
	public void addBar(LocalDate date, BigDecimal open, BigDecimal high, BigDecimal low, BigDecimal close) {
		Tbar bar = new Tbar(date,open,high,low,close);
		if(this.bars.size()==0) {
			bar.setTr(getTR(bar.getHigh(), bar.getLow(), bar.getClose()));
		}else {
			Tbar preBar = this.bars.get(this.bars.size()-1);
