...
@Service("turtleMusterSimulationByCompass")
public class TurtleMusterSimulationByCompass {
	@Value("${musterPath}")
	...
	Integer lostIndustryDruation = 5;
	Integer pool = 21;
	Integer top = 5;
	...
	public void simulate(LocalDate beginDate, LocalDate endDate) {
		...
		List<String> winIndustrys = new ArrayList<String>();
		...
		for(LocalDate date = beginDate; (date.isBefore(endDate) || date.equals(endDate)); date = date.plusDays(1)) {
			...
			if(musters!=null && musters.size()>0) {
				if(i%lostIndustryDruation==0 || first) {
					winIndustrys = compass.generateWinIndustrys(date);
					first = false;
					//System.out.println("win industrys: " + winIndustrys);
				}

				ms = new HashMap<String,Muster>();
				for(Map.Entry<String, Muster> entry : musters.entrySet()) {
					if(winIndustrys.contains(entry.getValue().getIndustry())) {
