...
@Service("b21plusH21")
public class B21plusH21 implements Producer{
	protected static final Logger logger = LoggerFactory.getLogger(B21plusH21.class);
	...
	private String fileName  = "B21plusH21.txt";
	...
	public List<String> produce(LocalDate date, boolean write) {
		...
		if(musters!=null && musters.size()>0) {
			...
			ms = tmps.subList(0, tmps.size()>=pool ? pool : tmps.size());    //最高价的前21只
